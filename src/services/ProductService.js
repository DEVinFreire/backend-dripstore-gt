...
class ProductService {
  async search(query) {
    ...
    const findOptions = { where: {}, include: [], distinct: true };
    ...
    if (priceRange) {
        ...
        findOptions.where.price = { [Op.between]: [min, max] };
    }
                ...
                where: {
                    ...
                    product_values: {
                        [Op.and]: optionValues.map(val => Sequelize.where(
                            Sequelize.fn('JSON_CONTAINS', Sequelize.col('product_values'), `"${val}"`), 1
                        ))
                    }
                },
            });
    ...
    try {
        ...
        if (options && Array.isArray(options)) {
            const optionPromises = options.map(opt => {
                const values = opt.value || opt.values;
            ...
            for (const option of options) {
                if (option.id && option.deleted) {
                    ...
                    await ProductOption.create({
                        ...
                        type: option.type,
                        product_values: option.value || option.values
                    }, { transaction });
                }
            }
        }
